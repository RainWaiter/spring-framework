...
public abstract class AdviceModeImportSelector<A extends Annotation> implements ImportSelector {
	...
	public final String[] selectImports(AnnotationMetadata importingClassMetadata) {
		// annType为例如 @EnableTransactionManagement注解类型 / @EnableCaching注解
		Class<?> annType = GenericTypeResolver.resolveTypeArgument(getClass(), AdviceModeImportSelector.class);
		// 得到@EnableTransactionManagement / @EnableCaching属性数据，例如： {order=2147483647, mode=PROXY, proxyTargetClass=false}
		...
		// 交给子类导出选择器class
		// 事务方面的子类是 ：TransactionManagementConfigurationSelector、
		// Cache缓存的子类是: CachingConfigurationSelector
